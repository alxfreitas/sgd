...
<script>
    $(document).ready(function(){
        ...
        $('#telefone').mask('(99) 9999-9999');
         ...
         showHideCongregacoes()
    });

    function showHideDiv(){
        if($('#naoEvangelico').attr('checked')==true){
            $('#dadosIgreja').hide();
        }else{
            $('#dadosIgreja').show();
        }
    ...
    function showHideCongregacoes(){
        if($('#belaVista').attr('checked')==true){
            $('#congregacoes').show();
        }else{
            $('#congregacoes').hide();
        }
...
<div class="fieldcontain ${hasErrors(bean: semeadorInstance, field: 'cpf', 'error')} required">
    ...
    <g:textField name="cpf" id="cpf" required="" value="${semeadorInstance?.cpf}"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: semeadorInstance, field: 'bairro', 'error')} ">
    ...
    <g:textField name="bairro" value="${semeadorInstance?.bairro}" style="text-transform: uppercase"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: semeadorInstance, field: 'estado', 'error')} ">
    ...
    <g:select name="estado" from="${semeadorInstance.constraints.estado.inList}" required=""
              value="${semeadorInstance?.estado}" valueMessagePrefix="estado.estado"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: semeadorInstance, field: 'cep', 'error')} ">
    ...
    <g:textField name="cep" id="cep" value="${semeadorInstance?.cep}"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: semeadorInstance, field: 'naoEvangelico', 'error')} ">
    ...
    <g:checkBox name="naoEvangelico" id="naoEvangelico" value="${semeadorInstance?.naoEvangelico}" onclick="showHideDiv()"/>
</div>
<div id="dadosIgreja">
    ...
    <div class="fieldcontain ">
        ...
        <g:checkBox name="belaVista" id="belaVista" onclick="showHideCongregacoes()"/>
    </div>
